print(
    "------------------------------U.S. HISTORY TRIVIA GAME------------------------------"
)
print()
print("                            Created by Mitchell Marczak\n")
print()
print(
    "The rules of this history trivia game are simple, answer using one of the options given, NO USING GOOGLE, make sure you are x-ed out of main.py and it says no files open so you don't see the code, and have fun. All you need to do is put the letter (a, b, c, or d) for the answer you think is right, make sure not to use capitals. Good luck and have fun! Now let's get started!\n"
)
...
while ready_check != yes:
    ...
    ready_check = input(
        "Are you ready to get started? (Answer 'yes' or 'no').\n")
    no = "no"
    yes = "yes"
    print()
g = open("Questions", "w")
...
read = open("Answers", "r")
answers_set = (read.read(1), read.read(1), read.read(1), read.read(1),
               read.read(1), read.read(1), read.read(1), read.read(1),
               read.read(1))
...
if users_answer in possible_choices:
    ...
    print(
        "You won't be able to redo this question this attempt, so choose an actual answer next time.\n"
    )
    incorrect_answers += 1
